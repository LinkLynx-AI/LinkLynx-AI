...
standard path の security baseline は、`edge + CI + audit posture` を低ノイズで成立させることを優先する。

### Baseline

- edge attach:
  - `Cloud Armor` baseline policy を `GCPBackendPolicy` で standard GitOps canary-smoke に attach
- CI security:
  - `Gitleaks`
  - `actions/dependency-review-action`
  - `Trivy config` against `infra/`
  - `Semgrep` on changed application files only
  - changed service images の `Trivy image`
- publish gate:
  - `LIN-966` の pushed image digest scan を継続
- manual DAST:
...
- audit baseline:
  ...
  - Terraform / GitOps / Argo CD sync history

### Why this shape

- standard path でも false positive が強い control を一気に merge-blocking にしない
- SAST は changed-files only にして、既存 backlog で全 PR が止まる状態を避ける
- DAST は evidence-first の manual workflow に留め、安定した authenticated smoke target ができてから強める
- control plane access hardening は current Terraform / Helm access model と結合しているため、この issue では exception を明文化する

詳細な verify / rollback は `docs/runbooks/security-standard-operations-runbook.md` を参照する。

## LIN-974 standard ops / capacity baseline

standard path の運用 baseline は、`staging` / `prod` の standard stack が増えても
最初の 15 分で迷わないことを優先して固定する。

### Baseline

- incident routing:
  - Discord thread
  - `hirwatan` / `sabe` / `miwasa` mention
  - `SEV-1` と sustained `SEV-2` では `Incident Commander` / `Comms owner` / `Scribe` を明示的に置く
- first-check sources:
  - `LIN-972` observability dashboard / alerts
  - `LIN-973` security attach / audit trail
  - Cloud SQL / Dragonfly / Scylla / managed messaging / search の各 standard runbook
- postmortem:
  - `SEV-1`
  - `30分超の SEV-2`
  - rollback / PITR / restore / reindex 実施時
  - 以上は `docs/runbooks/postmortem-standard-template.md` を必須化

### Initial planning envelopes
...
- WebSocket reconnect target:
  - rollout: `P95 <= 5s`
  - incident recovery: `P95 <= 10s`

### Capacity rule

- 登録者数単独では scale-out を決めない
- observed traffic、latency、DB/search pressure、dependency pressure、cost forecast を優先する
- optimize first、効かなければ scale、その後も採算が合わないなら architecture review を開く

### Chaos readiness

- 固定日では開始しない
- observability、rollback/tabletop、staging rehearsal、alert-noise control が揃ってから始める
- fault injection の前に tabletop / game-day を先に回す

incident / postmortem / capacity review の詳細は
`docs/runbooks/incident-standard-operations-runbook.md` を参照する。
