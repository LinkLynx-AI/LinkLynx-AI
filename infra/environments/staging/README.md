...
- incident / capacity baseline は `docs/runbooks/incident-standard-operations-runbook.md` を使う
- staging は tabletop / rollback rehearsal / restore decision dry-run の主な rehearsal 環境として使う
- `SEV-1` と sustained `SEV-2` では `Incident Commander` / `Comms owner` / `Scribe` を明示する
- scale trigger は registered user 数ではなく、WS concurrency / message ingress / latency / dependency pressure を見る
- incident close 後の standard postmortem は `docs/runbooks/postmortem-standard-template.md` を使う
