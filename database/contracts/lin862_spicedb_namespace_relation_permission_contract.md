...
4. `channel:{id}#guild@guild:{id}` tuple が存在する限り、guild baseline が channel permission へ伝播する。
5. SpiceDB timeout/transport error を unavailable として扱い、fail-open しない。
6. LIN-861 の現行 protected endpoint 2ケース（WS/session, protected ping）が object/permission に写像可能である。
