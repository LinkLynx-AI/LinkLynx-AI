...
python_supports_project() {
    ...
    [[ "$major" == "3" && -n "$minor" && "$minor" -ge "$MIN_PYTHON_MINOR" && "$minor" -le "$MAX_PYTHON_MINOR" ]]
}
...
select_python_for_project() {
    ...
    for python_cmd in "${candidates[@]}"; do
        if command -v "$python_cmd" >/dev/null 2>&1 && python_supports_project "$python_cmd"; then
            echo "$python_cmd"
            return 0
        fi
    done
...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
    echo -e "  ${CHECK} Docker Compose: ${GREEN}$(docker compose version --short 2>/dev/null || docker compose version | awk 'NR==1 {print $4}')${NC}"
    ((sys_ok++))
else
    echo -e "  ${CROSS} Docker Compose: ${RED}'docker compose' が利用できません${NC}"
fi
...
if command -v node >/dev/null 2>&1; then
    ...
    if [[ -n "$node_major" ]] && [[ "$node_major" -ge "$MIN_NODE_MAJOR" ]]; then
        echo -e "  ${CHECK} Node.js: ${GREEN}v${node_version}${NC}"
        ((ts_ok++))
    else
        echo -e "  ${WARN} Node.js: ${YELLOW}v${node_version}${NC} (${MIN_NODE_MAJOR}以上を推奨)"
    fi
else
    echo -e "  ${CROSS} Node.js: ${RED}未インストール${NC}"
fi
...
py_cmd=$(select_python_for_project || true)
if [[ -n "$py_cmd" ]]; then
    py_version=$($py_cmd --version 2>/dev/null | cut -d' ' -f2)
    echo -e "  ${CHECK} Python: ${GREEN}${py_version}${NC} (${py_cmd})"
    ((py_ok++))
elif command -v "python3" >/dev/null 2>&1; then
    py_version=$(python3 --version 2>/dev/null | cut -d' ' -f2)
    echo -e "  ${WARN} Python: ${YELLOW}${py_version}${NC} (3.${MIN_PYTHON_MINOR}-3.${MAX_PYTHON_MINOR} が必要)"
else
    echo -e "  ${CROSS} Python: ${RED}未インストール${NC}"
fi

if [[ -n "$py_cmd" ]] && "$py_cmd" -m pip --version >/dev/null 2>&1; then
    echo -e "  ${CHECK} pip: ${GREEN}$("$py_cmd" -m pip --version | awk '{print $2}')${NC}"
    ((py_ok++))
else
    echo -e "  ${CROSS} pip: ${RED}未インストール${NC}"
fi
...
if [[ -d "python/.venv" ]]; then
    if [[ -x "python/.venv/bin/python" ]]; then
        venv_minor=$(python/.venv/bin/python -c 'import sys; print(sys.version_info.minor)' 2>/dev/null)
        venv_version=$(python/.venv/bin/python --version 2>/dev/null | cut -d' ' -f2)
        if [[ -n "$venv_minor" ]] && [[ "$venv_minor" -le 13 ]]; then
            echo -e "  ${CHECK} 仮想環境 作成済み (${venv_version})"
        else
            echo -e "  ${WARN} 仮想環境: ${YELLOW}${venv_version}${NC} (Python 3.13以下で再作成を推奨)"
        fi
    else
        echo -e "  ${WARN} 仮想環境: ${YELLOW}Python実行ファイルが見つかりません${NC}"
    fi
elif [[ -n "$py_cmd" ]] && "$py_cmd" -m pip show fastapi >/dev/null 2>&1; then
    echo -e "  ${CHECK} 依存パッケージ インストール済み (グローバル)"
else
    echo -e "  ${WARN} 依存パッケージ: ${YELLOW}python/ で 'pip install -r requirements.txt -r requirements-dev.txt' を実行してください${NC}"
fi
...
if command -v elixir >/dev/null 2>&1; then
    ...
    if [[ -n "$elixir_version" ]] && version_ge "$elixir_version" "$MIN_ELIXIR_VERSION"; then
        echo -e "  ${CHECK} Elixir: ${GREEN}${elixir_version}${NC}"
        ((elixir_ok++))
    else
        echo -e "  ${WARN} Elixir: ${YELLOW}${elixir_version:-unknown}${NC} (${MIN_ELIXIR_VERSION}以上が必要)"
    fi
else
    echo -e "  ${CROSS} Elixir: ${RED}未インストール${NC}"
fi
...
# コンテナの起動状態確認
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1 && docker ps --format '{{.Names}}' 2>/dev/null | grep -q "postgres"; then
    echo -e "  ${CHECK} PostgreSQL: ${GREEN}起動中${NC}"
else
    echo -e "  ${WARN} PostgreSQL: ${YELLOW}停止中${NC} (実行: make db-up)"
fi

if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1 && docker ps --format '{{.Names}}' 2>/dev/null | grep -q "scylla"; then
