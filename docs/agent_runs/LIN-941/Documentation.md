...
- Now:
  ...
  - `0018_lin941_channel_category_contract` と `0020_lin941_channel_category_constraints` に分割し、enum 追加と constraint/trigger/index 更新の transaction 境界を分離した。
...
- 2026-03-08: local Postgres に対する `sqlx migrate run` で `unsafe use of new value "guild_category" of enum type channel_type` を検出し、enum-only migration と residual constraints migration に分離した
...
- 2026-03-13: `0017_lin939` / `0017_lin941` / `0017_lin948` の version collision を検出し、現行 repo の migration basename を `0017` / `0018` / `0019` / `0020` へ一意化した

## Review log
- 2026-03-08: reviewer fallback で `db-migrate` / `db-schema` / `db-schema-check` の成功証跡不足が block 指摘として上がった
- 2026-03-08: 上記 block に対し migration 分割と live DB 検証を実施し、再レビュー待ち状態へ更新した
