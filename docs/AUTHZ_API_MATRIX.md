...
| REST | `GET /v1/guilds/:guild_id/invites` | AuthN済み `principal_id` | `AuthzResource::Guild { guild_id }` | `View` | invite list path。optional `channel_id` filter を許可し、guild-wide list は channel metadata を返す。rate limit は `InviteAccess` を適用し、service 側で guild manage を fail-close 適用する |
...
| REST | `DELETE /v1/guilds/:guild_id/invites/:invite_code` | AuthN済み `principal_id` | `AuthzResource::Guild { guild_id }` | `Manage` | invite revoke path。optional `channel_id` filter を許可し、監査ログへ `principal_id/guild_id/invite_code/channel_id` を残す。rate limit は `InviteAccess` を適用する |
