...
`main` ブランチへの Pull Request 作成・更新時に、OpenAI API（GPT-4o）を用いたコードレビューが自動実行されます。レビュー結果は PR コメントとして投稿されます。
...
GitHub Actions: codex-review.yml
        ...
        │
        ├─ .github/scripts/codex_review.py を実行
        │       │
        │       ├─ OpenAI API (GPT-4o) へ diff を送信
        │       │
...
2. ログから原因を特定する
   ...
   | `openai.RateLimitError` | API のレート制限超過 | 数分待ってから再実行 |
   | `openai.AuthenticationError` | API キーが無効 | キーを再発行して更新 |
...
- PR が draft 状態でないか確認する（draft PR もトリガーされます）
...
`.github/workflows/codex-review.yml` の `branches` から `main` を削除するか、ファイル全体をコメントアウトする。
