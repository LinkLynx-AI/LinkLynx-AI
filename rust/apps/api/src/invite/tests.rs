...
mod tests {
    ...
    #[test]
    fn invite_error_invite_not_found_maps_to_not_found() {
        let error = InviteError::invite_not_found("invite_not_found");

        assert_eq!(error.status_code(), axum::http::StatusCode::NOT_FOUND);
        assert_eq!(error.app_code(), "INVITE_NOT_FOUND");
    }
    ...
    #[test]
    fn build_guild_invite_summary_preserves_creator_and_created_at() {
    ...
    #[test]
    fn list_invites_sql_returns_active_guild_scoped_invites() {
        ...
        assert!(sql.contains("i.max_uses IS NULL OR i.uses < i.max_uses"));
    ...
    #[test]
    fn revoke_invite_sql_disables_invite_and_writes_audit_log() {
        let sql = PostgresInviteService::REVOKE_INVITE_SQL;
    ...
    #[tokio::test]
    async fn postgres_list_invites_integration_returns_only_active_invites() {
        let Some((database_url, client)) = connect_integration_database().await else {
            return;
        };
        ...
        let active_invite_code = format!("LISTACTIVE{active_invite_id}");
        let expired_invite_code = format!("LISTEXPIRED{expired_invite_id}");
        let disabled_invite_code = format!("LISTDISABLED{disabled_invite_id}");

        seed_user(&client, owner_id, "invite-list-owner").await;
        client
            .execute(
                "INSERT INTO guilds (id, name, owner_id)
                 VALUES ($1, $2, $3)",
        ...
        client
            ...
            .await
            .expect("failed to seed invite list");
    ...
    #[tokio::test]
    async fn postgres_revoke_invite_integration_disables_invite_and_writes_audit_log() {
        let Some((database_url, client)) = connect_integration_database().await else {
            return;
        };
        ...
        let invite_row = client
            ...
            .await
            .expect("failed to read revoked invite");
        assert!(invite_row.get::<usize, bool>(0));

        let audit_row = client
            .query_one(
                "SELECT metadata->>'invite_code'
                 FROM audit_logs
                 WHERE guild_id = $1
                   AND actor_id = $2
                   AND action = 'INVITE_DISABLE'
                   AND target_id = $3
                 ORDER BY id DESC
                 LIMIT 1",
                &[&guild_id, &owner_id, &invite_id],
            )
            .await
            .expect("failed to read invite disable audit");
        assert_eq!(audit_row.get::<usize, Option<String>>(0), Some(invite_code));
    }
}
