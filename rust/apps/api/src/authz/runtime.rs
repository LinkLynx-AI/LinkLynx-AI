            ...
            match build_spicedb_runtime_config_from_env() {
                Ok(config) => {
                    ...
                    warn!(
                        ...
                        allow_all_until = %allow_all_until,
                        supported_action_count = supported_actions.len(),
                        "AUTHZ_PROVIDER=spicedb runtime foundation is configured, but provider implementation is pending; fallback to noop unavailable mode"
                    );
                }
                Err(reason) => {
                    warn!(
                        ...
                        allow_all_until = %allow_all_until,
                        supported_action_count = supported_actions.len(),
                        "AUTHZ_PROVIDER=spicedb runtime foundation is misconfigured; fallback to noop unavailable mode"
                    );
                }
            }
            Arc::new(NoopAllowAllAuthorizer::new(
